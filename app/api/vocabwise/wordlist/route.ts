...
export async function POST(req: NextRequest) {
  ...
  const { word, meaning_vi, pos, ipa, example_en, book_id, topic_id, topic_title } = await req.json()
  ...
  const { error } = await supabase.from('vw_saved_words').upsert(
    { family_id: session.familyId, word, meaning_vi, pos, ipa, example_en, book_id, topic_id, topic_title },
