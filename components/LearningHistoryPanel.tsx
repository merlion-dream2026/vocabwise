  ...
  return (
    <div className={`border-t border-gray-100 ${className ?? ''}`}>
      <button
        ...
        <span className="text-xs font-bold text-gray-400">📅 Lịch sử 30 ngày</span>
      ...
      {open && (
        <div className="px-3 pb-3 max-h-72 overflow-y-auto space-y-2.5">
          {days.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-1">Chưa có hoạt động trong 30 ngày qua</p>
          ) : days.map(h => (
            <div key={h.date}>
              <p className="text-[11px] font-black text-gray-400 mb-1">{h.label}</p>
