...
// ── AI writing-check daily usage (separate from the shared AI-text pool so heavy ──
// "Giải nghĩa" clicking during a lesson can't starve the end-of-topic writing grade) ──
const aiWritingStore = new Map<string, { count: number; resetAt: number }>()

export async function checkAndIncrementWritingCheckUsage(familyId: string, limit = 40): Promise<boolean> {
  const date = new Date().toISOString().split('T')[0]
  const key = `vw:ai-writing:${familyId}:${date}`
  if (redis) {
    try {
      const count = await redis.incr(key)
      if (count === 1) await redis.expire(key, 90000) // 25h
      return count <= limit
    } catch (e) { warnRedisFallback('checkAndIncrementWritingCheckUsage', e) }
  }
  const now = Date.now()
  const ttl = 25 * 60 * 60 * 1000
  let e = aiWritingStore.get(key)
  if (!e || now > e.resetAt) { e = { count: 0, resetAt: now + ttl }; aiWritingStore.set(key, e) }
  e.count++
  return e.count <= limit
}

// ── Batch sentence-grading usage (separate, smaller pool than the per-sentence ──
// writing-check quota — one submission grades several sentences in a single AI ──
// call, so a high-frequency quota isn't needed here). Shared by Module Test's ──
...
export async function checkAndIncrementSentenceGradingUsage(familyId: string, limit = 5): Promise<boolean> {
  ...
  if (redis) {
    try {
      ...
      if (count === 1) await redis.expire(key, 90000) // 25h
      return count <= limit
    } catch (e) { warnRedisFallback('checkAndIncrementSentenceGradingUsage', e) }
  }
  const now = Date.now()
  const ttl = 25 * 60 * 60 * 1000
  let e = sentenceGradingStore.get(key)
  if (!e || now > e.resetAt) { e = { count: 0, resetAt: now + ttl }; sentenceGradingStore.set(key, e) }
  e.count++
  return e.count <= limit
}
