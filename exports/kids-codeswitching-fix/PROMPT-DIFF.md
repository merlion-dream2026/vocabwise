...
Bạn đang audit phần giải thích từ vựng tiếng Anh (VocabWise Daily/Kids). Các file chunk trong
`exports/kids-codeswitching-fix/` liệt kê nhiều từ, mỗi từ format:
...
- `"I love bạn"` (trộn "I love" với "bạn")
- `"...và em cảm thấy rất happy."` (từ mục tiêu để nguyên cuối câu Việt)
- `"Brother, không quên mang sách về nhà nhé!"` (từ mục tiêu chèn đầu câu Việt)
- `"...luôn active khi còn nhỏ..."` (từ mục tiêu chèn giữa câu Việt)

Đọc **toàn bộ EXPLANATION** để hiểu đúng ngữ cảnh (không được đọc qua loa — nhiều từ có ví dụ lồng trong bullet mô tả ngữ cảnh, không phải lúc nào cũng có nhãn "Ví dụ:" rõ ràng).
...
NEW: "<câu tiếng Anh tự nhiên có dùng từ WORD>" (<bản dịch tiếng Việt tương ứng>)
```
...
**Không** viết lại các câu khác, không đổi văn phong, không thêm phần nào ngoài `OLD`/`NEW`.

- Nếu từ **không có lỗi** → bỏ qua hoàn toàn, không viết gì.
- 1 từ có thể có nhiều chỗ lỗi → xuất nhiều block `OLD`/`NEW` liên tiếp dưới cùng `[[word]]`, mỗi cặp cách nhau 1 dòng trống, ví dụ:
